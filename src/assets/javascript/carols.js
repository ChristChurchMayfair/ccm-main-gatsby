function show_more_events() {
    var laterEvents = document.querySelectorAll(
        "section.christmas2019 .events.later"
    )
    var i
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    var carolEvents = document.querySelectorAll(
        "section.christmas2019 .events.immediate .event"
    )
    var i
    ...
    var remainingCarolEvents = document.querySelectorAll(
        "section.christmas2019 .events.immediate .event"
    )
    var i
    for (i = 0; i < remainingCarolEvents.length; i++) {
        if (i >= numberOfEventsToLeaveVisible) {
            remainingCarolEvents[i].parentNode.removeChild(
                remainingCarolEvents[i]
            )
