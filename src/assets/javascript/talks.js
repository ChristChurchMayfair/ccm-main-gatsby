...
function showPodcastLink() {
    ...
    if (
        /* if we're on iOS, show the podcast links */
        isMac ||
...
function updatePageWithSermons(sermon_data) {
    ...
    for (series of sermon_data.data.allSeries) {
        ...
        var sermonSeriesDiv = document.importNode(
            sermonSeriesTemplate.content,
            true
        )

        //Determine if all sermons in this series relate to the same event.
        var sermonEvents = Array.from(
            new Set(series.sermons.map(sermon => sermon.event.name))
        )
        var event = null
        ...
        if (series.subtitle && series.subtitle != series.name) {
            sermonSeriesDiv.querySelector(".series-book").textContent =
                series.subtitle
        }
        sermonSeriesDiv.querySelector(".series-service").textContent = event
        sermonSeriesDiv
            .querySelector(".series-graphic")
            .setAttribute(
                "style",
                `background-image: url(${series.image3x2Url});`
            )
        ...
        for (sermon of series.sermons) {
            ...
            var dateString = new Date(
                Date.parse(sermon.preachedAt)
            ).toLocaleDateString()

            //Fill in sermon data.
            sermonDiv.querySelector(".sermon-title").textContent = sermon.name
            sermonDiv.querySelector(".sermon-speaker").textContent =
                sermon.speakers[0].name
            sermonDiv.querySelector(
                ".sermon-passage"
            ).innerHTML = `<a href="https://www.biblegateway.com/passage/?search=${sermon.passage}&version=NIVUK">${sermon.passage}</a>`
            sermonDiv.querySelector(".sermon-date").textContent = dateString
            sermonDiv
                .querySelector(".sermon-download")
                .setAttribute("href", sermon.url)
            sermonDiv
                .querySelector(".sermon-player")
                .setAttribute("src", sermon.url)
...
function setCurrentPageCSSClass(newCurrentPageNumber) {
    ...
    document
        .querySelectorAll(".current-pagelink")
        .forEach(function (pagelinkDiv) {
    ...
    document.querySelectorAll(".pagelink").forEach(function (pagelinkDiv) {
        ...
        if (
            links[0] &&
            parseInt(links[0].textContent) == newCurrentPageNumber
        ) {
